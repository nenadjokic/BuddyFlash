...
-- ============================================================
-- SECTION 7: AUTO-WHISPER MANAGEMENT
-- ============================================================

yOffset = yOffset - 170  -- space after quick add list

CreateSectionHeader(content, "Auto-Whisper on Login", 10, yOffset)
yOffset = yOffset - 10

local whisperInfoText = content:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
whisperInfoText:SetPoint("TOPLEFT", content, "TOPLEFT", 20, yOffset)
whisperInfoText:SetTextColor(0.6, 0.6, 0.6)
whisperInfoText:SetText("Automatically whisper a friend when they log in.\nUse BattleTag (e.g. Player#1234) or character name.")
whisperInfoText:SetJustifyH("LEFT")
yOffset = yOffset - 35

-- Whisper target input
local whisperTargetLabel = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
whisperTargetLabel:SetPoint("TOPLEFT", content, "TOPLEFT", 20, yOffset)
whisperTargetLabel:SetText("Friend:")

local whisperTargetInput = CreateFrame("EditBox", "BuddyFlashWhisperTarget", content, "InputBoxTemplate")
whisperTargetInput:SetPoint("TOPLEFT", content, "TOPLEFT", 80, yOffset)
whisperTargetInput:SetSize(180, 25)
whisperTargetInput:SetAutoFocus(false)
whisperTargetInput:SetMaxLetters(50)
whisperTargetInput:SetScript("OnEscapePressed", function(self) self:ClearFocus() end)
whisperTargetInput:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
yOffset = yOffset - 28

-- Whisper message input
local whisperMsgLabel = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
whisperMsgLabel:SetPoint("TOPLEFT", content, "TOPLEFT", 20, yOffset)
whisperMsgLabel:SetText("Message:")

local whisperMsgInput = CreateFrame("EditBox", "BuddyFlashWhisperMsg", content, "InputBoxTemplate")
whisperMsgInput:SetPoint("TOPLEFT", content, "TOPLEFT", 80, yOffset)
whisperMsgInput:SetSize(300, 25)
whisperMsgInput:SetAutoFocus(false)
whisperMsgInput:SetMaxLetters(200)
whisperMsgInput:SetScript("OnEscapePressed", function(self) self:ClearFocus() end)
whisperMsgInput:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
yOffset = yOffset - 30

-- Set / Remove whisper buttons
local setWhisperBtn = CreateButton(content, "Set Whisper", 20, yOffset, 120, 24, function()
    local target = whisperTargetInput:GetText():trim()
    local message = whisperMsgInput:GetText():trim()
    if target == "" or message == "" then
        print("|cFF69CCF0BuddyFlash:|r Enter both a friend name and message!")
        return
    end
    local db = ns.GetDB()
    db.autoWhisper[target] = message
    print("|cFF69CCF0BuddyFlash:|r Auto-whisper set for |cFFFFFF00" .. target .. "|r: \"" .. message .. "\"")
    whisperTargetInput:SetText("")
    whisperMsgInput:SetText("")
    whisperTargetInput:ClearFocus()
    whisperMsgInput:ClearFocus()
    RefreshWhisperList()
end)

local removeWhisperBtn = CreateButton(content, "Remove Whisper", 155, yOffset, 130, 24, function()
    local target = whisperTargetInput:GetText():trim()
    if target == "" then
        print("|cFF69CCF0BuddyFlash:|r Enter a friend name to remove!")
        return
    end
    local db = ns.GetDB()
    if db.autoWhisper[target] then
        ...
        print("|cFF69CCF0BuddyFlash:|r No auto-whisper found for |cFFFFFF00" .. target .. "|r")
    end
    whisperTargetInput:SetText("")
    whisperTargetInput:ClearFocus()
    RefreshWhisperList()
...
local whisperListFrame = CreateFrame("Frame", nil, content, "BackdropTemplate")
whisperListFrame:SetPoint("TOPLEFT", content, "TOPLEFT", 15, yOffset)
whisperListFrame:SetSize(430, 120)
whisperListFrame:SetBackdrop({
    bgFile = "Interface\\Tooltips\\UI-Tooltip-Background",
    edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
    tile = true, tileSize = 16, edgeSize = 12,
    insets = { left = 3, right = 3, top = 3, bottom = 3 },
})
whisperListFrame:SetBackdropColor(0, 0, 0, 0.4)
whisperListFrame:SetBackdropBorderColor(0.2, 0.3, 0.5, 0.6)

local whisperScroll = CreateFrame("ScrollFrame", "BuddyFlashWhisperScroll", whisperListFrame, "UIPanelScrollFrameTemplate")
whisperScroll:SetPoint("TOPLEFT", whisperListFrame, "TOPLEFT", 6, -6)
whisperScroll:SetPoint("BOTTOMRIGHT", whisperListFrame, "BOTTOMRIGHT", -24, 6)

local whisperScrollChild = CreateFrame("Frame", nil, whisperScroll)
whisperScrollChild:SetSize(400, 1)
whisperScroll:SetScrollChild(whisperScrollChild)

local whisperRows = {}

local function CreateWhisperRow(index)
    local row = CreateFrame("Frame", nil, whisperScrollChild)
    row:SetHeight(28)
    row:SetPoint("TOPLEFT", whisperScrollChild, "TOPLEFT", 0, -(index - 1) * 28)
    row:SetPoint("RIGHT", whisperScrollChild, "RIGHT", 0, 0)
    row:EnableMouse(true)
...
function RefreshWhisperList()
    ...
    for target, msg in pairs(db.autoWhisper) do
        table.insert(entries, { target = target, message = msg })
    end
    table.sort(entries, function(a, b) return a.target < b.target end)

    for i, entry in ipairs(entries) do
        local row = whisperRows[i] or CreateWhisperRow(i)
        row.nameText:SetText("|cFFFFFF00" .. entry.target .. "|r")
        row.msgText:SetText("\"" .. entry.message .. "\"")
        row.editBtn:SetScript("OnClick", function()
            whisperTargetInput:SetText(entry.target)
            whisperMsgInput:SetText(entry.message)
            whisperTargetInput:SetFocus()
        end)
        row.delBtn:SetScript("OnClick", function()
            db.autoWhisper[entry.target] = nil
            print("|cFF69CCF0BuddyFlash:|r Auto-whisper removed for |cFFFFFF00" .. entry.target .. "|r")
            RefreshWhisperList()
        end)
        row:Show()
    end

    for i = #entries + 1, #whisperRows do
        if whisperRows[i] then whisperRows[i]:Hide() end
    end

    if #entries == 0 then
        local row = whisperRows[1] or CreateWhisperRow(1)
        row.nameText:SetText("|cFF666666No auto-whispers set|r")
        row.msgText:SetText("")
        row.editBtn:Hide()
        row.delBtn:Hide()
        row:Show()
        whisperScrollChild:SetHeight(28)
    else
        whisperScrollChild:SetHeight(#entries * 28)
        for i = 1, #entries do
            whisperRows[i].editBtn:Show()
            whisperRows[i].delBtn:Show()
        end
    end
end
